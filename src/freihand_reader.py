...
class FreiHandReader:
    ...
    def get_frame_data(self, idx, strict: bool = False):
        """
        读取单帧数据。若 strict=False，单个样本失败会返回 None（不中断整体流程）。
        """
        try:
            ...
            mano_pose = raw_params[:48]
            mano_betas = raw_params[48:58]
            ...
            if strict:
                raise
            print(f"[WARN] get_frame_data 失败，idx={idx}，错误：{e}")
            return None
